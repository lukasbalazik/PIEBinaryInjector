...
{
    elf->ehdr.e_entry = inject->entry;
    return write_ehdr(elf);
}


int calculate_relative_address(elf_data_t *elf, inject_data_t *inject)
{
    int n;
    int off;
    unsigned long entry_point;
    unsigned long orig_ep_offset;

    off = lseek(elf->fd, 0, SEEK_END);
    n = (off % 4096) - (inject->secaddr % 4096);

    lseek(elf->fd, 24, SEEK_SET);
    read(elf->fd, &entry_point, sizeof(entry_point));
    
    orig_ep_offset = n + inject->secaddr + inject->offset_addr - entry_point;
    
    *(inject->code + inject->offset_ep_set) = orig_ep_offset & 0xFF;
    *(inject->code + inject->offset_ep_set+1) = (orig_ep_offset >> 8) & 0xFF;
    *(inject->code + inject->offset_ep_set+2) = (orig_ep_offset >> 16) & 0xFF;
    return 0;
}
...
int inject_code(int fd, inject_data_t *inject)
{
    ...
    if (calculate_relative_address(&elf, inject) < 0) {
        goto fail;
    }
